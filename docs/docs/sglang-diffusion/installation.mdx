...
The Instinct `mi30x` / `mi35x` tags do not include `gfx1151`. Build the TheRock image on the APU (`docker/rocm-gfx1151.Dockerfile`) and launch with **`torch_sdpa`**, not SRT `triton` or AITER:
...
docker/drun-rocm.sh \
  -e HF_TOKEN=<secret> \
  lmsysorg/sglang:dev-rocm-therock-gfx1151 \
...
Keep `--num-gpus 1`. Leave `SGLANG_USE_ROCM_VAE` unset (AITER GroupNorm is CDNA). Certify the image with a small image DiT first, then try other checkpoints on the APU. See [AMD GPUs](../hardware-platforms/amd_gpu) for host KMD, groups, and GTT.
