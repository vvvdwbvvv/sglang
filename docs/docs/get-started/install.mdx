...
1. Copy [compose.yaml](https://github.com/sgl-project/sglang/blob/main/docker/compose.yaml) (NVIDIA) or [compose.rocm.yaml](https://github.com/sgl-project/sglang/blob/main/docker/compose.rocm.yaml) (AMD ROCm) to your local machine. `lmsysorg/sglang:latest` is CUDA; ROCm tags are documented on [AMD GPUs](../hardware-platforms/amd_gpu).
2. Execute the command `docker compose up -d` (ROCm: `docker compose -f compose.rocm.yaml up -d`) in your terminal.
